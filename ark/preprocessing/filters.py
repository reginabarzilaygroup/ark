...
class FilterSliceThickness(Filter):
    def __init__(self, min_thick=0.0, max_thick=sys.maxsize):
        """Sets range for valid slice thickness values.

        Args:
            min_thick (float): Minimum valid slice thickness
            max_thick (float): Maximum valid slice thickness
        """
    ...
    def apply(self, dicoms):
        ...
        Returns:
            list: Filtered list of valid pydicom Datasets
        """
        valid = []
